...
class CombinedModel:
    ...
    def __init__(self, env):
        ...
        self.env = env
        self.low_model, self.cfg = self.load()
    ...
    def load(self):
        ...
        high_model_file = hf_hub_download(
                    repo_id="SaiResearch/booster_soccer_models",
                    filename="robot/lower_t1_control.pt",
                    repo_type="model")
        
        low_model = TD3.load(high_model_file, env=env, device="cpu")
        low_model.eval()

        low_model_file = hf_hub_download(
                    repo_id="SaiResearch/booster_soccer_models",
                    filename="robot/lower_t1_control.pt",
                    repo_type="model")
        
        low_model = torch.jit.load(low_model_file)
        low_model.eval()

        return low_model, cfg
    ...
    def get_actions(self, observation, info):
        ...
        if self.it % self.cfg["control"]["decimation"] == 0:
            dist = self.low_model(torch.tensor(obs.reshape(1,-1)))
